...
def generate_crosstab_report(unique_top_level_media_folders, mergerfs_disks_in_LtoR_order_from_fstab, mergerfs_disks_having_a_root_folder_having_files):
    ...
    def center_string(source_string, target_length):
        ...
        - target_length (int): The fixed size of the target string (must be even).
        Returns:
    ...
    for top_level_media_folder_name, top_level_media_folder_info in unique_top_level_media_folders.items():
        ...
        for disk in mergerfs_disks_in_LtoR_order_from_fstab:
            ...
            if has_info:
                ...
                # OK for this disk, within a top level media, folder now we have data we cn use
                row_number_of_files = row_number_of_files + number_of_files
                row_disk_space_used = row_disk_space_used + disk_space_used
                if number_of_files > 0 or disk_space_used_gb > 0 or is_ffd:
                   row_a.append(f"{number_of_files} files")
                   row_b.append(f"{disk_space_used_gb} Gb")
    ...
    # By the time we get to here, lists for the headings and the rows are populated.
    # Populate a list of max column lengths we can ue to format individual columns later.
    max_column_lengths = []
    num_columns = len(gigantic_list[0])
    max_column_lengths = [0] * num_columns      # initialize to zeroes
