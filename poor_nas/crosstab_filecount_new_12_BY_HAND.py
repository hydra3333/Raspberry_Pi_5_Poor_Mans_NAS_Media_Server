...
def generate_crosstab_report(unique_top_level_media_folders, mergerfs_disks_in_LtoR_order_from_fstab, mergerfs_disks_having_a_root_folder_having_files):
    ...
    #common_functions.debug_log_and_print("\n\nAPPENDED into 'gigantic_list' 'headers' (len={len(headers)}):\n", data=headers)
    ...
    for top_level_media_folder_name, top_level_media_folder_info in unique_top_level_media_folders.items():
        common_functions.debug_log_and_print(f"Top Level Media Folder Name: '{top_level_media_folder_name}' top_level_media_folder_info:\n", data=top_level_media_folder_info)
        ...
        for disk in mergerfs_disks_in_LtoR_order_from_fstab:
            ...
            if has_info:
                ...
                if disk_info:
                    ...
                    common_functions.debug_log_and_print(f"Top Level Media Folder Name: '{top_level_media_folder_name}' disk_mount_point: '{disk_mount_point}' Disk Info:\n", data=disk_info)
                    ...
                    common_functions.debug_log_and_print(f"Top Level Media Folder Name: '{top_level_media_folder_name}' disk_mount_point '{disk_mount_point}' does not have Disk Info for this top-level media folder:\n", data=disk_info)
                    is_ffd = False
                    root_folder_path = ""
                    number_of_files = 0
                    disk_space_used = 0
        ...
        #common_functions.debug_log_and_print("\n\nAPPENDED into 'gigantic_list' 'row_a' (len={len(row_a)}):\n", data=row_a)
        #common_functions.debug_log_and_print("\n\nAPPENDED into 'gigantic_list' 'row_b' (len={len(row_b)}):\n", data=row_b)
        #common_functions.debug_log_and_print("\n\nAPPENDED into 'gigantic_list' 'row_c' (len={len(row_c)}):\n", data=row_c)
        #common_functions.debug_pause()
    ...
    #common_functions.debug_log_and_print("\n\nAPPENDED into 'gigantic_list' 'free_space_row' (len={len(free_space_row)}):\n", data=free_space_row)
    #common_functions.debug_pause()
