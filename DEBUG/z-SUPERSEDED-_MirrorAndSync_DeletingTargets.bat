...
REM
REM
REM WARNING DESTRUCTION POSSIBLE
REM
REM    You are about to Mirror-synchronize files FROM '$sourcePath' INTO '$destPath'.
REM    This operation WILL OVERWRITE existing files where they have different sizes and
REM    WILL DELETE filenames in the destination which are not present in the source !
REM
REM
...
set "D6=W:\ROOTFOLDER6"
set "D7=Z:\ROOTFOLDER7"
...
REM note the DOUBLE BACKSLASH in the powershell commands if the powershell command inside a .bat file

REM assume X:\ROOTFOLDER1\ClassicDocumentaries is more up to date than F:\ROOTFOLDER3\ClassicDocumentaries
powershell -NoLogo -ExecutionPolicy Unrestricted -Sta -NonInteractive -Command "& {Get-ChildItem -Path 'X:\\ROOTFOLDER1\\ClassicDocumentaries' -File | ForEach-Object { $_.CreationTime = (Get-Date).Date.AddHours(01); $_.LastWriteTime = (Get-Date).Date.AddHours(01) }}"
powershell -NoLogo -ExecutionPolicy Unrestricted -Sta -NonInteractive -File "X:\CONVERT\z_MirrorAndSync_DeletingTargets.ps1"
pause
goto :eof
...
REM assume X:\ROOTFOLDER1\ClassicMovies is more up to date than F:\ROOTFOLDER3\ClassicMovies
powershell -NoLogo -ExecutionPolicy Unrestricted -Sta -NonInteractive -Command "& {Get-ChildItem -Path 'X:\\ROOTFOLDER1\\ClassicMovies' -File | ForEach-Object { $_.CreationTime = (Get-Date).Date.AddHours(01); $_.LastWriteTime = (Get-Date).Date.AddHours(01) }}"
REM pause
REM call :do_robo "X:\ROOTFOLDER1\ClassicMovies" "F:\ROOTFOLDER3\ClassicMovies"
REM pause
...
REM /L means ONLY list the proposed actions
REM set "RoboSwitches=/L /MIR /IT /COPY:DAT /DCOPY:DAT /XJD /XJF /E /Z /J /TIMFIX /TS /R:10 /W:2 /XO /FP /NDL /ETA /TEE"
set "RoboSwitches=/L /IT /COPY:DAT /DCOPY:DAT /XJD /XJF /E /Z /J /TIMFIX /TS /R:10 /FP /ETA /TEE /XO /NDL"
REM
...
REM echo about to: robocopy "%dst%" "%src%" %RoboSwitches%
REM robocopy "%dst%" "%src%" %RoboSwitches%
goto :eof
