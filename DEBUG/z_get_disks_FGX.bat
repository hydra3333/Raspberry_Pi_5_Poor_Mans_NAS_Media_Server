...
echo COPY /Y "!path2!\z_get_disks.py"  "!path1!\"
COPY /Y "!path2!\z_get_disks.py"  "!path1!\"

echo COPY /Y "!path2!\z_get_disks.py"  "!path3!\"
COPY /Y "!path2!\z_get_disks.py"  "!path3!\"
...
for %%p in ("%path1%" "%path2%" "%path3%") do (
    ...
    REM echo %cd%
    REM cd
    REM Inner loop to process paths
    for %%q in ("%path1%" "%path2%" "%path3%") do (
        echo +++++ Start  Running python3 with CD in folder "%%~p" from .py file using: "%py_exe%" "%%~q\z_get_disks.py"
        echo CD
        CD
        "%py_exe%" "%%~q\z_get_disks.py"
        echo ----- Finish Running python3 with CD in folder "%%~p" from .py file using: "%py_exe%" "%%~q\z_get_disks.py"
        echo.
    )
)
echo ------------------------------------------------------------------------------------------------------------------------------
